...
/*   Updated: 2025/05/06 09:50:09 by ipetrov          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 *	RETURN VALUES
 *		If OKful,
			the pthread_create() function will return zero.
			Otherwise an ER number will be returned to indicate the ER.
 *	ERS
 *		The pthread_create() function will fail if:

			*		[EAGAIN]           The system lacked the necessary
			resources to create another thread,
...
t_sts	th_create(pthread_t *tid, void *func(void *), void *arg)
{
	...
	if (code == 0)
		...
		return (puterr("pthread_create: Error: Resource temporarily \
			unavailable\n"));
	else if (code == EPERM)
...
/**
 *	RETURN VALUES
 *		If OKful, the pthread_join() function will return zero.  Otherwise,
			an ER number will be returned to indicate the ER.
 *	ERS
